...
#define NUM_TD_PER_THREAD 4 // number of poses per work item
...
__attribute__((reqd_work_group_size(256,1,1))) //how many work items in work group
...
__kernel void fasten_main(const int natlig,
                          ...
                          __global FFParams *restrict global_forcefield,
                          ...
                          const int numTransforms)
{
  ...
  //printf("ocl kernel index is %d natlig %d natpro %d \n",ix,natlig,natpro); 
...
__local FFParams forcefield[34];
  ...
  event_t event = async_work_group_copy((__local float *)forcefield,   
					(__global float *)global_forcefield,                       
                                       34*sizeof(FFParams)/sizeof(float),
                                        0);
  ...
  do
  {
    ...
    do
    {
      ...
      const float r_radij = native_divide(ONE,radij);
      ...
      const float r_distdslv = native_divide(ONE,distdslv);
